...
def buttons_for_seller_item(user, id_item, type_item):
    buttons = [
        [
            ...
            InlineKeyboardButton(
                _("set location"), callback_data="location&%s&%s" % (type_item, id_item)
            ),
        ],
        [
            InlineKeyboardButton(
                _("show card"), callback_data="show&%s&%s" % (type_item, id_item)
            ),
            InlineKeyboardButton(
                _("show on map"), callback_data="on_map&%s&%s" % (type_item, id_item)
            ),
        ],
...
def buttons_for_purchase_item(user, id_item, media=None, has_media=None):
    ...
    if not media:
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    _("change_seller"), callback_data="show_change_seller&%s" % id_item
                ),
                InlineKeyboardButton(
                    _("change_category"),
        ...
        if has_media:
            ...
            buttons.append(
                [
                    InlineKeyboardButton(
                        _("Change media"), callback_data="set_media&" + str(id_item)
                    )
                ]
            )
        else:
            buttons.append(
                [
                    InlineKeyboardButton(
                        _("Set media"), callback_data="set_media&" + str(id_item)
                    )
                ]
            )
    else:
        buttons = [
            [
                InlineKeyboardButton(
                    _("Card"), callback_data="show&purchase&" + str(id_item)
                )
            ],
        ]
...
def get_button_main():
    new_button = InlineKeyboardButton(_("Menu"), callback_data="menu")
    keyboard = InlineKeyboardMarkup([[new_button]])
    return keyboard


def get_button_geo():
    geo = KeyboardButton(_("location"), request_location=True)
    keyboard = ReplyKeyboardMarkup([[geo]])
    return keyboard


def get_button_register():
    yes_button = InlineKeyboardButton(_("Yes"), callback_data="register")
    no_button = InlineKeyboardButton(_("No"), callback_data="no_register")
...
def get_button_menu(user_id):
    buttons = [
        [
            InlineKeyboardButton(_("categories"), callback_data="list_of&category"),
...
def get_button_users():
    ...
    for user in users:
        ...
        row_but.append(
            InlineKeyboardButton(user.username, callback_data="show&user&%s" % user.id)
        )
        row_but.append(
            InlineKeyboardButton(
                str(user.is_active), callback_data="show&user&%s" % user.id
            )
        )
...
def get_button_lang():
    ...
    for l in LANGUAGES:
        ...
        buttons.append(InlineKeyboardButton(l, callback_data="lang&%s" % l))
    keyboard = InlineKeyboardMarkup([buttons])
    return keyboard
...
def get_button_sellers(user, id_item, geo=None, radius=RADIUS_SEARCH_SELLER):
    ...
    purchase = Purchase.get(Purchase.id == id_item, Purchase.user == user)
    sellers = []
    ...
    for seller in sellers:
        ...
        buttons.append(
            InlineKeyboardButton(
                sel_name,
                callback_data="change_seller&%s&%s&%s" % ("purchase", id_item, sel_id),
            )
        )
    ...
    menu.append([new_button])
    new_button = InlineKeyboardButton(
        _("New Seller"), callback_data="new_seller&purchase&%s" % purchase.id
    )
    menu.append([new_button])
    new_button = InlineKeyboardButton(_("Menu"), callback_data="menu")
    menu.append([new_button])
...
def get_button_categories(user, id_item, type_item):
    ...
    for category in categories:
        ...
        if count == 5:
            ...
            buttons.append(
                InlineKeyboardButton(
                    cat_name,
                    callback_data="change_category&%s&%s&%s"
                    % (type_item, id_item, category.id),
                )
            )
        else:
            count += 1
    ...
    menu.append(buttons)
    new_button = InlineKeyboardButton(
        _("New Category"), callback_data="new_category&%s&%s" % (type_item, id_item)
    )
    menu.append([new_button])
...
def get_button_list_items(user, type_obj, page=1):
    ...
    list_obj = list_obj.order_by(dict_types[type_obj].id.desc()).paginate(
        page, COUNT_ON_PAGE
    )
    buttons = []
    for obj in list_obj:
        ...
        columns = [
            InlineKeyboardButton(
                "X", callback_data="delitem&%s&%s" % (type_obj, str(obj.id))
            )
        ]
    ...
    if total_count > 10:
        ...
        columns = [
            InlineKeyboardButton(prev_text, callback_data=callback_data_prev),
            InlineKeyboardButton(next_text, callback_data=callback_data_next),
        ]
        buttons.append(columns)
    ...
    if type_obj != "purchase":
        ...
        new_button = InlineKeyboardButton(txt_btn, callback_data="new_%s" % type_obj)
        buttons.append([new_button])
    new_button = InlineKeyboardButton(_("Menu"), callback_data="menu")
    buttons.append([new_button])
    keyboard = InlineKeyboardMarkup(buttons)
...
def get_columns_items(columns, type_obj, obj):
    if type_obj == "purchase":
        ...
        columns.append(
            InlineKeyboardButton(
                "%s" % obj.datetime, callback_data="show&purchase&" + str(obj.id)
            )
        )
        columns.append(
            InlineKeyboardButton(
                _("Media"), callback_data="show_picture&purchase&%s" % str(obj.id)
            )
        )
    elif type_obj == "seller":
        columns.append(
            InlineKeyboardButton(obj.name, callback_data="show&seller&" + str(obj.id))
        )
    elif type_obj == "category":
        columns.append(
            InlineKeyboardButton(obj.name, callback_data="show&category&" + str(obj.id))
        )
    return columns
...
def get_button_orders():
    buttons = [
        [
            InlineKeyboardButton(_("by category"), callback_data="order_by&category"),
...
def get_button_del_item(id, type):
    buttons = [
        [
            InlineKeyboardButton(
                _("Delete"), callback_data="delitem&%s&%s" % (type, str(id))
            )
        ]
    ]
    keyboard = InlineKeyboardMarkup(buttons)
...
def get_button_confirm(id):
    buttons = [
        [
            InlineKeyboardButton(
                _("Yes"), callback_data="confirm&purchase&%s&yes" % id
            ),
...
def get_button_order_by(user, type_c):
    ...
    list_month_of_reports = months[
        (
            0
            if month_now - count_month_reports < 0
            else month_now - count_month_reports
        ) : month_now
    ]
    print("MONTHS: ", list_month_of_reports)
    if len(list_month_of_reports) < count_month_reports:
        list_month_of_reports = (
            months[month_now - count_month_reports : 12] + list_month_of_reports
        )
    ...
    for c in list_by_for:
        ...
        if c.name:
            c_name = c.name
        rows.append(
            InlineKeyboardButton(c_name, callback_data="%s&%s" % (command, c_name))
        )
        for m in list_month_of_reports:
            month = str(m)
            if m < 10:
            ...
            summ = (
                Purchase.select(fn.SUM(Purchase.summ))
                .where(
                    by_for_field == c,
                    Purchase.user == user,
            ...
            rows.append(
                InlineKeyboardButton(
                    summ, callback_data="%s&%s&%s" % (command, c_name, m)
                )
            )
        buttons.append(rows)
    new_button = InlineKeyboardButton(_("Menu"), callback_data="menu")
...
def get_button_purchases_by(user, name, by_what, month=None):
    ...
    for p in purchases:
        ...
        rows.append(
            InlineKeyboardButton(
                _("Media"), callback_data="show_picture&purchase&%s" % str(p.id)
            )
        )
        buttons.append(rows)
    new_button = InlineKeyboardButton(_("Menu"), callback_data="menu")
