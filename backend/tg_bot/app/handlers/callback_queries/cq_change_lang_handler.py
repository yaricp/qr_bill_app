...
async def cq_change_lang_handler(query: CallbackQuery, callback_data: LangsData):
    ...
    if result != user_lang:
        ...
        mess = await bot.send_message(
            chat_id=user_id,
            text=result_lang_view(lang=result["lang"], changed=True),
            parse_mode="HTML",
        )
        ...
        mess = await bot.send_message(
            chat_id=user_id, text=result_lang_view(lang=user_lang), parse_mode="HTML"
        )
        await bot.delete_message(chat_id=user_id, message_id=query.message.message_id)
