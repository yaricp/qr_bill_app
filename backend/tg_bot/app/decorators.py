                ...
                if user and user.is_active:
                    ...
                    if args[0].callback_query:
                        args[0].callback_query.answer(
                            "Sorry! you need to register. Use /start for this."
                        )
                    else:
                        obj.reply_text(
                            "Sorry! you need to register. Use /start for this."
...
def is_not_bot():
    def wrap(f):
        def wrapped_f(*args, **kwargs):
            obj = None
            # if DEVEL: print('args[1]: ', args[1].__dict__)
            # if DEVEL: print('args[0]: ', args[0].__dict__)
            if args[0].message:
                obj = args[0].message
            elif args[0].callback_query:
                obj = args[0].callback_query
    ...
    def wrap(f):
        def wrapped_f(*args, **kwargs):
            ...
            if obj:
                ...
                if user_lang.lang:
                    lang = user_lang.lang
                lang_user = gettext.translation(
                    "messages", localedir="lang", languages=[lang]
                )
                lang_user.install()
                return f(*args, **kwargs)
