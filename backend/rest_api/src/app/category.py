...
class CategoryQueries:
    ...
    async def get_all_categories(self, user_id: UUID):
        return CategoryORM.query.filter_by(user_id=user_id).all()

    async def get_category(self, id: UUID, user_id: UUID):
        return CategoryORM.query.filter_by(id=id, user_id=user_id).first()
    ...
    async def count_goods_by_name(
        ...
        if first_of:
            if delta_month != 1:
                ...
                result = (
                    db_session.query(
                        CategoryORM.name, func.sum(GoodsORM.quantity).label("count")
                    )
                    .join(GoodsORM.bill)
                    .join(GoodsORM.user_product)
                    .join(UserProductORM.categories)
                    .filter(
                ...
                result = (
                    ...
                    .join(GoodsORM.user_product)
                    .join(UserProductORM.categories)
                    .filter(CategoryORM.user_id == user_id, GoodsORM.user_id == user_id)
                    .group_by(CategoryORM.name)
                    .order_by(desc("count"))
                    .limit(first_of)
                )
...
class CategoryCommands:
    ...
    async def get_or_create(
        ...
        cat = await self.get_by_name(incoming_item=incoming_item)
        if not cat:
            cat = await self.create_category(incoming_item=incoming_item)
        return cat
    ...
    async def update_category(self, incoming_item: CategoryUpdate) -> Category:
    ...
    async def delete_category(self, id: UUID) -> Category:
        cat = CategoryORM.query.filter_by(id=id).first()
