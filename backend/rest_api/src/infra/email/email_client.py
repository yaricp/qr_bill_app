...
from .config import app_config, smtp_server_config
...
class EmailClient:
    ...
    def __init__(
        ...
        self.templated_message = Template(
            template=template, template_vars=self.template_vars
        ).render()
        if self.templated_message:
            self.logger.info(f"self.templated_message: {self.templated_message}")
    ...
    def send(self):
        ...
        try:
            ...
            mail_from = {"name": app_config.APP_NAME, "email": self.sender_email}
            recipients = [{"name": "Client", "email": self.email}]
