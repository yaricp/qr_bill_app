...
from .grpc_pb2 import (BillInfo, LoginURL, TgUserID, TotalSumm, UserLang,
                       UserLangEditForm)
...
class RestApiGRPC(grpc_pb2_grpc.RestApiGRPCServicer):
    ...
    def __init__(self):
        self.address = (
            f"{grpc_server_config.GRPC_SERVER_HOST}"
    ...
    async def SendBillUrl(self, request, context: grpc.aio.ServicerContext) -> BillInfo:
        ...
        #     summ=0.0
        # )
        user = await self.user_commands.register_user_by_tg_id(tg_id=request.tg_user_id)
        if user:
            income_bill = BillCreateByURL(link=request.url, image="")
    ...
    async def GetTotalSumm(
        ...
        total_summ = await self.bill_commands.get_total_summ(user_id=request.tg_user_id)
        return TotalSumm(summ=total_summ)
    ...
    async def GetUserLang(self, request, context: grpc.aio.ServicerContext) -> UserLang:
        user = self.user_queries.get_user_by_tg_id(tg_id=request.tg_user_id)
        return UserLang(lang=user.lang)

    async def SetUserLang(self, request, context: grpc.aio.ServicerContext) -> UserLang:
        logger.info("SetUserLang")
        user_db = self.user_queries.get_user_by_tg_id(tg_id=request.tg_user_id)
        user_income = UserUpdate(id=user_db.id, lang=request.lang)
        logger.info(f"user_income: {user_income}")
        user = await self.user_commands.edit_user(user_income)
        logger.info(f"changed user lang: {user.lang}")
        return UserLang(lang=user.lang)

    async def GetLoginURL(self, request, context: grpc.aio.ServicerContext) -> LoginURL:
        url = await self.user_commands.create_temp_login_link(tg_id=request.tg_user_id)
...
async def serve():
    ...
    grpc_pb2_grpc.add_RestApiGRPCServicer_to_server(rest_api_grpc, server)
