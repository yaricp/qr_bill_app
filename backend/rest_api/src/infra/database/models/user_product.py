...
class UserProduct(Model):
    ...
    product_id = Column(UUID, ForeignKey(Product.id), nullable=True)
    product = relationship("Product")

    categories: Mapped[Set["Category"]] = relationship(
        secondary=user_product_category, back_populates="user_products"
    )

    def __repr__(self):
        return (
            f"UserProduct {self.product.name} with id - <{self.id}>"
