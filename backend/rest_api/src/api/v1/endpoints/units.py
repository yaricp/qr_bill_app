...
@app.get(app_config.REST_API_PREFIX + "/units/", tags=["Units"], response_model=List[Unit])
async def get_all_units_route(user=Depends(manager)) -> List[Unit]:
    """Gets list of units"""
    return await get_all_units()


@app.get(app_config.REST_API_PREFIX + "/units/{id}", tags=["Units"], response_model=Unit)
...
@app.put(app_config.REST_API_PREFIX + "/units/{id}", tags=["Units"], response_model=Unit)
...
@app.delete(app_config.REST_API_PREFIX + "/units/{id}", tags=["Units"], response_model=Unit)
