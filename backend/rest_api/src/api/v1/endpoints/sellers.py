...
from src.api.v1.services.seller import (create_seller, delete_seller,
                                        get_all_sellers, get_seller,
...
@app.post(
    app_config.REST_API_PREFIX + "/sellers/", tags=["Sellers"], response_model=Seller
)
...
@app.get(
    app_config.REST_API_PREFIX + "/sellers/",
    tags=["Sellers"],
...
async def get_all_sellers_route(
    ...
    return await get_all_sellers(user_id=user.id, offset=offset, limit=limit)


@app.get(
    app_config.REST_API_PREFIX + "/sellers/{id}",
    tags=["Sellers"],
...
async def update_seller_route(id: UUID, item_in: SellerUpdate, user=Depends(manager)):
    """Updates seller info"""
    seller: Seller = await update_seller(id=id, seller_data=item_in)
    if not seller:
        raise HTTPException(status_code=404, detail="Seller not found")
    return seller
