...
async def update_product_route(
    ...
    product: Product = await update_product(id=id, product_data=item_in)
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
    return product
...
async def get_uncategorized_product_route(
    ...
    return await get_uncategorized_product(user_id=user.id, cat_id=cat_id)
...
async def save_categorized_goods_route(
    ...
    """
    return await save_categorized_products(cat_prod_data=data_in)
...
async def update_product_categories_route(
    ...
    return await update_product_categories(
        user_product_id=data_in.user_product_id, list_cat_id=data_in.list_cat_id
    )
...
async def product_prices_route(
    ...
    return await get_product_prices(product_id=product_id, user_id=user.id)
...
async def products_for_prices_route(user=Depends(manager)) -> List[Product]:
    """Gets products that have more than one price"""
    return await get_products_more_one_prices(user_id=user.id)
