...
def check_user_auth(email_login_tg_link: str | UUID) -> User | None:
    ...
    if isinstance(email_login_tg_link, UUID):
        user = users_queries.get_user_by_login_link(link=str(email_login_tg_link))
        if user:
            return user
    ...
    user = users_queries.get_user_by_login(login=str(email_login_tg_link))
    ...
    try:
        user = users_queries.get_user_by_tg_id(tg_id=int(email_login_tg_link))
        return user
...
async def register_new_user(user_data: UserCreate) -> User:
    ...
    command_result = await user_commands.register_user_by_login(incoming_item=user_data)
    return command_result


async def create_login_password_user(user_id: UUID, user_data: UserCreate) -> User:
    user_commands: UserCommands = UserCommands()
...
async def verify_email_tg(link: UUID) -> UUID | None:
    user_commands: UserCommands = UserCommands()
    return await user_commands.verify_email_tg(link=link)
