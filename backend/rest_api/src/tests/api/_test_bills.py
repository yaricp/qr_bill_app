...
from src.infra.database.models import (  # Убедись, что у тебя есть такие модели в проекте
    Bill, Seller, User)
...
def test_update_bill(test_client, auth_headers, db_session):
    ...
    response = test_client.put(
        f"/api/v1/bills/{bill_id}", json=update_data, headers=auth_headers
    )
...
def test_parse_url_bill(test_client, auth_headers):
    ...
    data = {
        "link": "https://mapr.tax.gov.me/ic/#/verify?iic=B1EEF0F469C52D9B26E869841CB73700&tin=03224678&crtd=2024-12-28T17:10:14%2001:00&ord=68288&bu=oj275xi131&cr=bu831zx287&sw=on241rl293&prc=36.80"
    }
    ...
    if response.status_code == 200:
        ...
        assert response.json()["seller"] == {
            "name": '"GUŠTI" D.O.O.',
            "address": "NJEGOŠEVA BR.139(C-VRAČAR) - H. NOVI",
        }
...
def test_get_uncategorized_goods(test_client, auth_headers, db_session):
    ...
    response = test_client.get(
        f"/api/v1/bills/{bill_id}/uncategorized_goods/", headers=auth_headers
    )
    assert response.status_code == 200
    assert isinstance(response.json(), list)


def test_get_uncategorized_goods_by_category(test_client, auth_headers, db_session):
    """Тест получения некатегоризированных товаров по `bill_id` и `cat_id`"""
    bill_id, _ = create_test_data(db_session)
    cat_id = str(uuid4())

    response = test_client.get(
        f"/api/v1/bills/{bill_id}/uncategorized_goods/{cat_id}", headers=auth_headers
    )
    assert response.status_code == 200
    assert isinstance(response.json(), list)
...
def test_get_uncategorized_products(test_client, auth_headers, db):
    ...
    response = test_client.get(
        f"/api/v1/bills/{bill_id}/uncategorized_products/", headers=auth_headers
    )
    assert response.status_code == 200
    assert isinstance(response.json(), list)
...
def test_get_uncategorized_products_by_category(test_client, auth_headers, db):
    ...
    response = test_client.get(
        f"/api/v1/bills/{bill_id}/uncategorized_products/{cat_id}", headers=auth_headers
    )
    assert response.status_code == 200
    assert isinstance(response.json(), list)
...
def test_get_month_summary(test_client, auth_headers):
    ...
    delta_month = 3
    response = test_client.get(
        f"/api/v1/bills/month_summ/{delta_month}", headers=auth_headers
    )
