...
from app.bill import BillQueries, BillCommands
...
async def test_get_all_bills_with_offset_and_limit(monkeypatch):
    ...
    mock_query.filter_by.return_value.offset.return_value.limit.return_value.all.return_value = ['bill1', 'bill2']
...
async def test_get_month_summ(monkeypatch):
    ...
    monkeypatch.setattr('app.bill.get_fisrt_day_month_by_delta_month', lambda x: '2024-01-01')
    monkeypatch.setattr('app.bill.get_last_day_of_month_by_datetime', lambda x: '2024-01-31')
    mock_query = MagicMock()
...
async def test_get_data_from_fiscal_api_success(monkeypatch):
    ...
    result = await commands.get_data_from_fiscal_api(iic="1", tin="2", crtd="2024-01-01 00:00:00")
