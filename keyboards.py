...
def get_button_sellers(user, id_item, geo=None):
    ...
    curs.execute("""\
        ...
        FROM seller, (SELECT ST_transform(ST_SetSRID(ST_MakePoint(%s, %s),4326),3857)::geography AS poi) AS f
        WHERE ST_DWithin(ST_transform(ST_SetSRID(ST_MakePoint(seller.longitude, seller.latitude),4326),3857), 
        poi, 1000);""", poi)
